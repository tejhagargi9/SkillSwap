    ...
    const fetchRequests = async () => {
      try {
        ...
        const filtered = allRequests.filter(
          (req) => req.recipientUserId._id === userId && req.status === "pending"
        );
    ...
    <div className="w-full sticky top-0 z-10">
      ...
      <div className="w-full bg-white shadow-md px-6 flex justify-between items-center h-20">
        ...
        <div className="flex space-x-8">
          ...
          <Link to="/working" className="text-gray-700 hover:text-indigo-600 font-medium">How It Works</Link>
          <Link to="/skillMatching" className="text-gray-700 hover:text-indigo-600 font-medium">Skill Match</Link>
          <Link to="/chat" className="text-gray-700 hover:text-indigo-600 font-medium">Chatting</Link>
        </div>
        ...
        <div className="flex items-center space-x-4">
                ...
                <div className="absolute right-0 mt-3 w-96 bg-white rounded-xl shadow-xl overflow-hidden z-20 border border-gray-100">
                  <div className="py-2">
                    <div className="px-4 py-3 bg-gradient-to-r from-indigo-500 to-indigo-600 flex justify-between items-center">
                      <h3 className="text-sm font-semibold text-white">Skill Requests</h3>
                      <button 
                    ...
                    {notifications.length === 0 ? (
                      ...
                      <div className="max-h-72 overflow-y-auto">
                        {notifications.map(notification => (
                          <div key={notification.id} className="px-4 py-4 border-b border-gray-100 hover:bg-gray-50 m-2 rounded-lg shadow-sm">
                            <div className="flex">
                              ...
                              <div className="flex-grow">
                                ...
                                <p className="text-xs text-gray-500 mb-3">Requested: <span className="font-medium">{notification.skill}</span></p>
          ...
          {isLoggedIn ? (
            <Link to="/account" className="px-4 py-2 bg-indigo-600 text-white font-medium rounded-md hover:bg-indigo-700 transition duration-150">Account</Link>
          ) : (
            <>
              <Link to="/login" className="px-4 py-2 text-indigo-600 font-medium hover:text-indigo-800 transition duration-150">Log In</Link>
              <Link to="/signup" className="px-4 py-2 bg-indigo-600 text-white font-medium rounded-md hover:bg-indigo-700 transition duration-150">Sign Up</Link>
            </>
          )}
        </div>
      </div>
